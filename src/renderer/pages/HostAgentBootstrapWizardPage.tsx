...
export function HostAgentBootstrapWizardPage() {
  ...
  const createToken = async () => {
    const json = await apiFetch<ApiResponse<BootstrapResult>>('/api/host-agents/bootstrap-tokens', {
      method: 'POST',
      body: JSON.stringify({
        workspaceId,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<button onClick={() => navigate('/host-agents')} className="px-4 py-2 rounded-md bg-[hsl(var(--muted))]">返回列表</button>}
      />
      ...
      <SectionCard title="Step 2 · 生成 Bootstrap Token" description="默认 15 分钟有效；注册成功后即失效。">
        <button onClick={() => void createToken()} className="px-4 py-2 rounded-md bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] hover:opacity-90">
        ...
        <>
          <SectionCard title="Step 3 · 安装命令" description="把 `<soloForge-host>` 替换成远端宿主机能访问到的 SoloForge 地址；默认 Local API 仍只监听 127.0.0.1。">
            <div className="space-y-3">
              ...
              <div className="text-sm text-[hsl(var(--muted-foreground))]">过期时间：{formatDateTime(result.expiresAt)}</div>
              <pre className="text-xs font-mono whitespace-pre-wrap p-4 rounded-md bg-[hsl(var(--muted))] overflow-auto">{result.installCommand}</pre>
              <button onClick={() => void copyCommand()} className="px-4 py-2 rounded-md bg-[hsl(var(--muted))] hover:opacity-90">{copied ? '已复制' : '复制命令'}</button>
            </div>
          </SectionCard>

          <SectionCard title="Step 4 · 注册完成确认" description="Agent 启动成功后会自动注册并开始 heartbeat；随后回到 Host Agents 页面刷新即可看到 ONLINE。">
            <button onClick={() => navigate('/host-agents')} className="px-4 py-2 rounded-md bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] hover:opacity-90">
              返回 Host Agents 检查心跳
            </button>
          </SectionCard>
        </>
