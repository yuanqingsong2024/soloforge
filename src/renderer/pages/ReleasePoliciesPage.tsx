    ...
    <div className="space-y-6">
      ...
      <SectionCard title="新建策略">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input value={form.name} onChange={e => setForm(prev => ({ ...prev, name: e.target.value }))} placeholder="策略名称" className="px-3 py-2 rounded-workshop-md border border-[hsl(var(--border))] bg-[hsl(var(--background))]" />
          ...
          <textarea value={form.targetScopeJson} onChange={e => setForm(prev => ({ ...prev, targetScopeJson: e.target.value }))} rows={4} className="px-3 py-2 rounded-workshop-md border border-[hsl(var(--border))] bg-[hsl(var(--background))] font-mono text-xs" />
          <textarea value={form.releaseChannelScopeJson} onChange={e => setForm(prev => ({ ...prev, releaseChannelScopeJson: e.target.value }))} rows={4} className="px-3 py-2 rounded-workshop-md border border-[hsl(var(--border))] bg-[hsl(var(--background))] font-mono text-xs" />
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.autoDetectUpdates} onChange={e => setForm(prev => ({ ...prev, autoDetectUpdates: e.target.checked }))} /> 自动发现更新</label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requireBackup} onChange={e => setForm(prev => ({ ...prev, requireBackup: e.target.checked }))} /> 强制备份</label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requireApproval} onChange={e => setForm(prev => ({ ...prev, requireApproval: e.target.checked }))} /> 强制审批</label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requireMaintenanceWindow} onChange={e => setForm(prev => ({ ...prev, requireMaintenanceWindow: e.target.checked }))} /> 强制维护窗口</label>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.allowAutoRollback} onChange={e => setForm(prev => ({ ...prev, allowAutoRollback: e.target.checked }))} /> 允许自动回滚</label>
        </div>
